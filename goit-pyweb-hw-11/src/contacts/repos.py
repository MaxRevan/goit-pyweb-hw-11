...
class ContactRepository:
    ...
    async def update_contact(self, contact_id: int, contact_data: ContactCreate) -> Contact:
        query = select(Contact).where(Contact.id == contact_id)
        result = await self.session.execute(query)
    ...
    async def get_upcoming_birthdays(self) -> List[Dict]:
        upcoming_birthdays = []
        today = date.today()
        query = select(Contact).where(Contact.birthday != None).where(
                (Contact.birthday >= today) & 
                (Contact.birthday <= today + timedelta(days=7))
                )
        result = await self.session.execute(query)
